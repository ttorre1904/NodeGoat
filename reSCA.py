...
def main(input_path, output_path):
    ...
    for card in cards:
        ...
        package_manager_text = package_manager.text.split(':')[-1].strip() if package_manager else "No package manager found"
        ...
        overview = card.find_next('h2', id='overview')
        if overview:
            overview_content = []
            for sibling in overview.find_next_siblings():
                if sibling.name == 'h2':
                    break
                if sibling.name == 'p':
                    overview_content.append(sibling.text)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python script.py <input_html_path> <output_xlsx_path>")
    else:
